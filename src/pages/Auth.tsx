...
export default function Auth() {
  ...
  // Idempotent admin bootstrap on first load
  useEffect(() => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const emailToUse = loginEmail.trim().toLowerCase() === ADMIN_USERNAME ? ADMIN_EMAIL : loginEmail;
    ...
    <div className="flex min-h-screen items-center justify-center bg-background p-4">
      <div className="w-full max-w-[420px] border border-border rounded-md p-8 space-y-6">
        ...
        <div className="flex flex-col items-start gap-3">
          ...
          <p className="text-[13px] text-muted-foreground">Track, manage, and optimize your IT assets</p>
        </div>
          ...
          <TabsContent value="login" className="mt-4">
            <form onSubmit={handleLogin} className="space-y-3">
              <div className="space-y-1">
                <Label className="text-[12px]">Email or username</Label>
                <Input type="text" placeholder="you@example.com or administrator" value={loginEmail} onChange={(e) => setLoginEmail(e.target.value)} required className="h-8 text-[13px]" />
              </div>
              <div className="space-y-1">
