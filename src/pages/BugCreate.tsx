...
export default function BugCreate() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!trimmed.title || !trimmed.description) {
      toast({ title: "Missing fields", description: "Asset name and description are required.", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    try {
      ...
      toast({ title: "Asset added!", description: `Asset ID: ${data.tracking_id}` });
      navigate("/");
    } catch (error: any) {
    ...
    <AppLayout>
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center gap-2 px-4 md:px-6 h-11 border-b border-border shrink-0">
          ...
          <h1 className="text-[13px] font-medium">Add an Asset</h1>
        </div>

        <div className="flex-1 overflow-auto">
          <form onSubmit={handleSubmit} className="max-w-2xl">
            <div className="px-4 md:px-6 py-4 border-b border-border space-y-1">
              <Label className="text-[12px] text-muted-foreground">Asset Name *</Label>
              <Input
                placeholder='e.g. MacBook Pro 16" — Engineering'
                value={form.title}
            ...
            <div className="px-4 md:px-6 py-4 border-b border-border space-y-1">
              ...
              <Textarea
                placeholder="Brand, model, serial number, and any relevant details"
            ...
            <div className="px-4 md:px-6 py-4 border-b border-border space-y-1">
              <Label className="text-[12px] text-muted-foreground">Acquisition Notes</Label>
              <Textarea
                placeholder="Purchase date, vendor, PO number, warranty info..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              <div className="px-4 md:px-6 py-4 border-b border-border md:border-r space-y-1">
                <Label className="text-[12px] text-muted-foreground">Specifications</Label>
                <Textarea
                  placeholder="CPU, RAM, storage, OS version..."
              ...
              <div className="px-4 md:px-6 py-4 border-b border-border space-y-1">
                <Label className="text-[12px] text-muted-foreground">Current Condition</Label>
                <Textarea
                  placeholder="Any wear, damage, or known issues?"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
              <div className="px-4 md:px-6 py-4 border-b border-border md:border-r space-y-1">
                <Label className="text-[12px] text-muted-foreground">Criticality *</Label>
              ...
              <div className="px-4 md:px-6 py-4 border-b border-border space-y-1">
                <Label className="text-[12px] text-muted-foreground">Location / Department</Label>
                <Input
                  placeholder="e.g. HQ — Engineering, Floor 3"
